...
STAGES: list[Stage] = [
    Stage(
        ...
        activities=[
            ...
            Activity(
                ...
                params=[
                    Param(name="asin", label="ASIN", placeholder="B0XXXXXXXX", help="Amazon ASIN (positional arg)"),
                    Param(name="category", label="Category", kind="select", flag="--category",
                          choices=["", "Audio", "Automotive", "Baby & Kids", "Beauty", "Books",
                          ...
                          help="Required (no interactive prompt in the UI)"),
                    Param(name="name", label="Name override (optional)", flag="--name",
    ...
    Stage(
        ...
        activities=[
            ...
            Activity(
                id="deals_facebook",
                title="Post Deals to Facebook",
                description="Scrape Amazon Today's Deals and post the top deals to the Facebook Page.",
                cmd=["python", "scripts/7_deal_finder.py"],
                cwd="growth-engine",
                long_running=True,
                danger=True,
                params=[
                    Param(name="post_facebook", label="Post to Facebook (off = scrape only)",
                          kind="bool", flag="--post-facebook"),
                    Param(name="dry_run", label="Dry run (fixture data, no network/post)",
                          kind="bool", flag="--dry-run"),
                    Param(name="min_discount", label="Min discount %", flag="--min-discount",
                          placeholder="25"),
                    Param(name="categories", label="Categories (comma-sep, optional)",
                          flag="--categories", placeholder="kitchen,fitness"),
                ],
            ),
        ],
    ),
