export default function RatingStars({ rating, size = "sm" }: { rating: number; size?: "sm" | "md" | "lg" }) {
  ...
  return (
    <span className="flex items-center gap-0.5" aria-label={`${rating} out of 5 stars`}>
      {[1, 2, 3, 4, 5].map((i) => {
        ...
        return (
          <svg key={i} className={`${sz} ${filled || half ? "text-amber-400" : "text-zinc-700"}`} viewBox="0 0 20 20" fill="currentColor">
            {half ? (
              <>
                <defs>
